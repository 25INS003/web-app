...
export const useProductStore = create((set, get) => ({
  ...
  updateProduct: async (shopId, productId, productData) => {
    set({ isLoading: true, error: null });
    try {
      const response = await apiClient.put(`/${shopId}/products/${productId}`, productData);
  ...
  deleteProduct: async (shopId, productId) => {
    set({ isLoading: true, error: null });
    try {
      await apiClient.delete(`/${shopId}/products/${productId}`);
  ...
  uploadProductImages: async (shopId, productId, formData) => {
    ...
    try {
      const response = await apiClient.post(
        `/${shopId}/products/${productId}/images`, 
        formData, 
  ...
  getProductDetails: async (shopId, productId) => {
    ...
    // 2. If not, fetch it (Re-using list endpoint with search or ID param, 
    //    OR you might need a specific GET /:id endpoint depending on backend)
    try {
        ...
        const response = await apiClient.get(`/${shopId}/products?product_id=${productId}`);
